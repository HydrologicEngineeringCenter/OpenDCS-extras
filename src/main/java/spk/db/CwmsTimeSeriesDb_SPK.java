/*
*
* $Id: CwmsTimeSeriesDb_SPK.java,v 1.2 2014/05/22 12:17:15 mmaloney Exp $
* 
...
*  
*  $Log: CwmsTimeSeriesDb_SPK.java,v $
...
*  Modify the CwmsTimeSeriesDb_SPK with adding the writeTsGroup() to avoid using the one in TimeSeriesDb. This will solve the issue that cp_comp_depends gets updated while a TS group is saved into CWMS DB.
*
			...
			if (minRecNum != -1)
			{
				...
				if (sinceTime != null)
					q = q + " AND a.START_DATE_TIME > " + sqlDate(sinceTime);
                                        ...
                                        // lets try just letting the system always process 20,000 reocrds. This could make things 
                                        // better when processing has stalled and other such instances.
					q = q + " AND ROWNUM < 20000"; //+ " and a.record_num between " + minRecNum + " and " + maxRecNum;
                                }
				q = q + failTimeClause;
...
//		this.tasklistQueueThresholdHours = thresholdHours;
	...
	{
		return new spk.db.CwmsTimeSeriesDAO_SPK(this, dbOfficeId);
	}
