 ...
 */
//AW:JAVADOC_END   
public class Camanche_2017Tkt1222
...
//AW:TIMESLICE                
                ...
                try{
                    ...
                    double rain_gross_res = graph.gross_pool() - tcs_rain;
                    double non_transfer_res = graph.gross_pool() - non_transferable_space;
                    ...
                    double gross_reservation = Math.max( rain_gross_res, snow_gross_res ); 
                    ...
                    double salt_springs_rain_credit = Double.NEGATIVE_INFINITY;
                    if( salt_springs <= max_rain_salt_springs ){
                         salt_springs_rain_credit = Math.min( 
                                percent_salt_springs*transferable_res, //3a2
                                salt_springs_space_available //3a1
                        );
                    } else {
                        salt_springs_rain_credit = percent_salt_springs*transferable_res; //3a2 (from if less than 90k clause)
                    }
                    
                    
                    //secton 3 b
                    double lower_bear_space_available = Math.max(0 , max_rain_lower_bear - lower_bear);
                    double lower_bear_rain_credit = Double.NEGATIVE_INFINITY;
                    if( lower_bear < max_rain_lower_bear ){
                        lower_bear_rain_credit = Math.min(
                                percent_lower_bear*transferable_res, //3b2
                                lower_bear_space_available //3b1
                        );
                    } else {
                        lower_bear_rain_credit = percent_lower_bear*transferable_res; //3b2 (from if less than clause )
                    }
                    ...
                    double salt_springs_snow_credit = Math.min( 
                            ...
                            percent_salt_springs*snow_gross_res // section 4 a 2                            
                            );
                    // section 4 b
                    double lower_bear_snow_credit = Math.min(
                             snow_percent*(max_snow_lower_bear-(lower_bear+lower_bear_rain_credit)), // section 4 b 1
                             percent_lower_bear*snow_gross_res // section 4 b 2
                             
                            );
                    
                    // section 4 c
                    double snow_credit = 0;
                    if( wy_day >= dates.March15 && wy_day < dates.August01 ){
                        snow_credit = lower_bear_snow_credit + salt_springs_snow_credit;
                    }
                    // section 5
