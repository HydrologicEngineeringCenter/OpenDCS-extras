...
//AW:TIMESLICE
		...
		if (!isMissing(speed) && !isMissing(dir))
		{
                    ...
                    _u = speed * Math.cos(r);
                    _v = speed * Math.sin(r);
                    debug2(String.format( "Converted input spd,deg (%f.02,%.02f) to u,v (%.04f,%.04f)", speed,dir,_u,_v));
                    u.add(_u);
                    v.add(_v);
...
//AW:AFTER_TIMESLICES
		...
		if (count >= minSamplesNeeded)
		{
                        ...
                        debug2( String.format("Average spd,dir (%.02f,%.02f) from avg u,v (%.04f,%.04f)", avg_speed,avg_dir, u_avg,v_avg) );
