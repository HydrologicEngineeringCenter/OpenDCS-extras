...
//AW:JAVADOC
/**
 * Reads shift information from and file and applies the number to the
...
//AW:TIMESLICE
        ...
        double pzf = this.getPZF(_timeSliceBaseTime, base_units); // TODO: needs a unit mechanism
...
//AW:AFTER_TIMESLICES_END
     ...
     *
     * @param sliceTime time for the current value
     * @return the shift value appropriate for this time
     */
    public double getShift(Date sliceTime) {
        /**
         * return the nearest previous time shift value otherwise return 0.0;
         * assuming there is no shift
         *
         *
         */
        ...
        if (map == null) {
            return 0.0; // if there is no config, we assume no shift
        }
    ...
    public double getPZF(Date sliceTime, String to_units) {
        ...
        if (d != null) {
            ...
            try {
                ...
                Logger.getLogger(ApplyShift.class.getName()).log(Level.SEVERE, null, e);
                return _d;
            }
