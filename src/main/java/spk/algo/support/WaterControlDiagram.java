...
/**
 * Contains all of the support functions for calculating top of conservation
 * storage
 *
 * @author L2EDDMAN
 */
public class WaterControlDiagram {
     ...
     *
     * @param day The day you are calculating Top of Conservation for
     *
     * @param fcp The Current Flood Control Paramenter
     * @param snow Are we calculating snow melt
     * @return The allowable storage in the reservoir for the day
    ...
    /**
     * Interpolate along a curve represented by one of the label lines on a
    ...
    /**
     * Bound the value of Conservation storage based on the extremums provided
     * in the water control manual. If you are porting from HADA you should also
     * check the DIFL file and confirm with the water manager for the reservoir
     *
     * @param day water year julian day
     * @param value The value to check the bounds on
     * @return if the value is within the bounds of the graph, the value will be
     * returned, otherwise the upper or lower bound value will be returned.
     ...
     *
     * @param day the water year julian day
     * @param curve array of doubles that contain the actual values, must match
     * with times
    ...
    public double interpolate_curve(int day, Double[] curve) throws InterpolationException, ArrayIndexOutOfBoundsException {
        ...
        double value = ((curve[index_day_after] - curve[index_day_before])
                / (_times[index_day_after] - _times[index_day_before]))
                * (day - _times[index_day_before])
                + curve[index_day_before];
        ;
    ...
    public static IrrigationDemands get_irrigation_data(String filename) throws Exception {
