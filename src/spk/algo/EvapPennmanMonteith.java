...
//AW:LOCALVARS
	...
	public double evap = 0.0; //output value
        ...
        //public double delta_saturation_vapor_pressure = 0.0; // kPa/degC
        //public double psychrometric_constant = 0.0; // kPa/degC  ( lamda symbol in ASCE documents )
        /*
...
//AW:TIMESLICE
		// Enter code to be executed at each time-slice.
                ...
                if( isMissing( AirTemp ) && isMissing(RelativeHumidity) && isMissing( WindSpeed) && isMissing(SolarRadiation) && isMissing(BaroPressure) )
                {
                    return;
                }

                debug3("Inputs this time slice:\t  input  |  value ");
                debug3("                       \t AirTemp |  "+ AirTemp);
                debug3("                       \t  RH     |  "+ RelativeHumidity);
                debug3("                       \t WindSpd |  "+ WindSpeed);
                debug3("                       \t SolRad  |  "+ SolarRadiation);
                debug3("                       \t Pressure|  "+ BaroPressure);
		...
		if( !isMissing( AirTemp ))
                {
                    ...
                    if(!isMissing(RelativeHumidity))
                    {
                        ...
                        debug3("e_a now: " + mean_actual_vapor_pressure + " \t added " + (RelativeHumidity/100)*e_naught( AirTemp )   );
...
//AW:AFTER_TIMESLICES
		...
		// period.
                debug3("****Calculating evap for period****");
                ...
                debug3( " gamma (kPa/C):" + psychrometric_constant );
                debug3( " delta (kPa/C):" + delta );
                ...
                debug3( " Total Solar Rad  (MJ/m^2/day)  : " + total_radiation/1000000.0);
                ...
                double Rn = Rns - Rnl;
                debug3( " Net Radiation (MJ/m^2/day): " + Rn);
                ...
                debug3( " denomenator Calc: " + denomenator);


                evap = numerator/denomenator;
                evap = Math.max(0,evap); // evap is never negative
                debug3(" Final Evap Calc(mm): " + evap );
                ...
                if( count >= MinSamples )
                {
                    ...
                    try {
                        ...
                        setFlagBits(Evap, flags);
                            ...
                            debug3("failed to convert to desired units");
                            throw new DbCompException( "No Conversion" );
                    }
                }
                else
                {
                    warning("There where not enough values present, assuming the evap number calculated is junk, sorry" );
                }
