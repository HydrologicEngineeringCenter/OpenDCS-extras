/**
 * $Id: CwmsTimeSeriesDAO.java,v 1.3 2014/07/03 12:43:23 mmaloney Exp $
 * 
...
public class CwmsTimeSeriesDAO_SPK 
	extends decodes.cwms.CwmsTimeSeriesDAO
//	implements TimeSeriesDAI
{
	...
	protected CwmsTimeSeriesDAO_SPK(DatabaseConnectionOwner tsdb, String dbOfficeId)
	{
...
debug3("using display name '" + displayName + "', unique str='" + uniqueString + "'");
		...
		if (ts_code == Constants.undefinedId)
			throw new NoSuchObjectException("No CWMS Time Series for ID '"
				...
				+ ((CwmsTimeSeriesDb_SPK)db).getDbOfficeId());
...
debug3("After re-getting tsid dn='" + cts.getDisplayName() + "'");
		...
		throws DbIoException, BadTimeSeriesException
	{
		String q = "select DATE_TIME, ROUND(VALUE,8), QUALITY_CODE "
			...
			+ " AND UNIT_ID = " + sqlString(ts.getUnitsAbbr())
			+ " and date_time = "
			+   "(select max(date_time) from CWMS_V_TSV_DQU_30D "
			+   	"where TS_CODE = " + ts.getSDI()
